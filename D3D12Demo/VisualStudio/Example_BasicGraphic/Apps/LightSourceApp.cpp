...
void CLightSourceApp::InitRenderResource()
{
...

	{
		...
		m_pVSShaderCode_Light = m_pGraphicContext->CompileShader("D:\\Projects\\MyProjects\\LearnDirectX12\\D3D12Demo\\Shaders\\light_color.fx", "VSMain", "vs_5_0");
		m_pPSShaderCode_Light = m_pGraphicContext->CompileShader("D:\\Projects\\MyProjects\\LearnDirectX12\\D3D12Demo\\Shaders\\light_color.fx", "PSMain", "ps_5_0");
	}
...
void CLightSourceApp::UpdateFrameBuffer(float fDeltaTime, float fTotalTime)
{
	...
	if (m_DirLights.size() + m_PointLights.size() + m_SpotLights.size() <= 16)
	{
		...
		for (int i = 0; i < m_SpotLights.size(); ++i)
		{
			...
			lightInfo.RefDist = pLight->m_fRefDist;
			lightInfo.MaxRadius = pLight->m_fMaxRadius;
			lightInfo.CosMinAngle = std::cosf(MathUtility::ToRadian(pLight->m_fMinAngle));
			lightInfo.CosMaxAngle = std::cosf(MathUtility::ToRadian(pLight->m_fMaxAngle));
...
void CLightSourceApp::BuildStaticMeshes(ID3D12Device* pDevice, ID3D12GraphicsCommandList* cmdList)
{
	...
	// Plane
	{
		...
		pBoxMesh->m_pPositionBufferGPU = m_pGraphicContext->CreateDefaultBuffer(&positions[0], 4 * sizeof(XMFLOAT3));
		pBoxMesh->m_PositionBufferView = m_pGraphicContext->CreateVertexBufferView(pBoxMesh->m_pPositionBufferGPU, 4 * sizeof(XMFLOAT3), sizeof(XMFLOAT3));

		pBoxMesh->m_pNormalBufferGPU = m_pGraphicContext->CreateDefaultBuffer(&normals[0], 4 * sizeof(XMFLOAT3));
		pBoxMesh->m_NormalBufferView = m_pGraphicContext->CreateVertexBufferView(pBoxMesh->m_pNormalBufferGPU, 4 * sizeof(XMFLOAT3), sizeof(XMFLOAT3));
		
		pBoxMesh->m_pIndexBuferGPU = m_pGraphicContext->CreateDefaultBuffer( &indices[0], 6 * sizeof(UINT16));
		pBoxMesh->m_IndexBufferView = m_pGraphicContext->CreateIndexBufferView(pBoxMesh->m_pIndexBuferGPU, 6 * sizeof(UINT16), DXGI_FORMAT_R16_UINT);
...
void CLightSourceApp::BuildScene()
{
	...
	
	{
		...
		if (pSphereMesh)
		{
			...
			pObj->m_Transform.Position = XMFLOAT3(5.0f, 1.0f, 0.0f);
...

	{
		...
		if (pSphereMesh)
		{
			CRenderObject* pObj = new CRenderObject();
			pObj->m_pStaticMesh = pSphereMesh;
			pObj->m_Transform.Position = XMFLOAT3(10.0f, 1.0f, 0.0f);
...

	{
		...
		pLight0->m_Color = XMFLOAT3(0.3f, 0.3f, 0.3f);
		...
		pLight1->m_Color = XMFLOAT3(0.7f, 0.7f, 0.7f);
		...
		pSpotLight0->m_Color = XMFLOAT3(0.333f, 0.333f, 0.333f);
		...
		pSpotLight1->m_Color = XMFLOAT3(0.667f, 0.667f, 0.667f);
